class Sudoku < ActiveRecord::Base
	...
	def init
		board_s = self.my_board
		#board_s = "003020600900305001001806400008102900700000008006708200002609500800203009005010300"
		@board_a = board_s.split("").map { |s| s.to_i }
		#@board_a = my_board.split("").map { |s| s.to_i }
	...
	def solve
		...
		if(@board_a.include?(0)==false)
			board_strings = @board_a.map { |i| i.to_s }
			sol_string = board_strings.join("")
			self.solution = sol_string
